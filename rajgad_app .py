...
# सर्व १२ महिन्यांची क्रमवार यादी
MONTHS_LIST = [
    "जानेवारी",
    "फेब्रुवारी",
    "मार्च",
...
st.markdown(
    '<div class="main-title">🏰 राजगड गृहनिर्माण संस्था (Housing Society)</div>',
    unsafe_allow_html=True,
)
st.markdown(
    '<div class="sub-title">मेंटेनन्स, खर्च आणि फ्लॅटनिहाय थकबाकी व्यवस्थापन'
    " प्रणाली</div>",
    unsafe_allow_html=True,
)
...
if menu == "📊 डॅशबोर्ड (हिशोब सारांश)":
  ...
  with c1:
    ...
    if not df_maint.empty:
      st.dataframe(
          df_maint.sort_values(by="id", ascending=False).head(5)[[
              "flat_no",
              "month_year",
              "amount",
              "payment_mode",
  ...
  with c2:
    ...
    if not df_exp.empty:
      st.dataframe(
          df_exp.sort_values(by="id", ascending=False).head(5)[[
              "title",
              "category",
              "amount",
              "payment_mode",
...
# ================= 2. फ्लॅट व सभासद नोंदणी =================
  ...
  tab1, tab2 = st.tabs(
      ["नवीन फ्लॅट / सभासद जोडा", "नोंदणीकृत फ्लॅट्स (Edit/Delete)"]
  )
  ...
  with tab1:
    with st.form("add_flat_form", clear_on_submit=True):
      ...
      m_maint = col2.number_input(
          "मासिक मेंटेनन्स आकारणी (₹):",
          min_value=0.0,
          value=1000.0,
          step=100.0,
      )
      ...
      if btn_add:
        if f_no and o_name:
          try:
            c = conn.cursor()
            c.execute(
                "INSERT INTO flats (flat_no, owner_name, contact,"
                " monthly_maint) VALUES (?, ?, ?, ?)",
  ...
  with tab2:
    ...
    if not df_flats.empty:
      ...
      flat_to_edit = st.selectbox(
          "फ्लॅट निवडा:", df_flats["flat_no"].tolist()
      )
      flat_curr = df_flats[df_flats["flat_no"] == flat_to_edit].iloc[0]

      with st.form("edit_flat_form"):
        new_owner = st.text_input(
            "सभासदाचे नाव:", value=flat_curr["owner_name"]
        )
        new_contact = st.text_input(
            "मोबाईल नंबर:", value=flat_curr["contact"] or ""
        )
        new_maint = st.number_input(
            "मासिक मेंटेनन्स (₹):",
            value=float(flat_curr["monthly_maint"]),
            step=100.0,
        )
        ...
        if btn_update:
          c = conn.cursor()
          c.execute(
              "UPDATE flats SET owner_name=?, contact=?, monthly_maint=? WHERE"
              " flat_no=?",
...
# ================= 3. मेंटेनन्स जमा =================
  ...
  if df_flats.empty:
    ...
    with st.form("maint_form", clear_on_submit=True):
      ...
      col2.text_input(
          "सभासदाचे नाव:", value=flat_info["owner_name"], disabled=True
      )

      col3, col4 = st.columns(2)
      curr_year = datetime.now().year
      all_possible_months = [f"{m} {curr_year}" for m in MONTHS_LIST]
      sel_month = col3.selectbox(
          "कोणत्या महिन्याचे शुल्क भरले?", all_possible_months
      )
      rec_amt = col4.number_input(
          "रक्कम (₹):", value=float(flat_info["monthly_maint"]), step=100.0
      )

      col5, col6 = st.columns(2)
      pay_mode = col5.selectbox(
          "पैसे कसे मिळाले? (Payment Mode):",
          ["ऑनलाइन (GPay / PhonePe / NEFT)", "रोख (Cash)", "धनादेश (Cheque)"],
      )
      rec_date = col6.date_input("पावती तारीख:")
      ...
      if btn_save_maint:
        ...
        conn.commit()
        st.success(
            f"फ्लॅट क्र. {sel_flat} साठी {sel_month} चे मेंटेनन्स नोंदवले गेले!"
        )
        st.rerun()

    st.markdown("---")
    st.write("### 📜 सर्व पावत्यांची यादी")
    ...
    if not df_maint_all.empty:
      ...
      with st.expander("🗑️ चुकीची पावती हटवा (Delete Receipt)"):
        del_rec_id = st.selectbox(
            "पावती ID निवडा:", df_maint_all["id"].tolist()
        )
        if st.button("पावती डिलीट करा", type="primary"):
          c = conn.cursor()
          c.execute(
              "DELETE FROM maintenance_received WHERE id=?", (del_rec_id,)
          )
          conn.commit()
...
# ================= 4. सोसायटी खर्च नोंद =================
  ...
  with st.form("expense_form", clear_on_submit=True):
    col1, col2 = st.columns(2)
    exp_title = col1.text_input("खर्चाचे नाव (उदा. वॉटर टँकर, लाईट बिल):").strip()
    exp_cat = col2.selectbox(
        "खर्चाचा प्रवर्ग:",
        [
    ...
    exp_mode = col6.selectbox(
        "पेमेंट मोड:", ["ऑनलाइन (UPI/NEFT)", "रोख (Cash)", "धनादेश (Cheque)"]
    )
  ...
  df_exp_all = pd.read_sql_query(
      ...
      " (₹)', expense_date as 'तारीख', paid_to as 'कोणास दिले', payment_mode as"
      " 'पद्धत' FROM expenses ORDER BY id DESC",
      conn,
  )
...
# ================= 5. फ्लॅटनिहाय सविस्तर थकबाकी अहवाल =================
  ...
  st.subheader("⚠️ फ्लॅटनिहाय एकूण थकबाकी व बाकी महिन्यांची यादी")
  ...
  if df_flats.empty:
    ...
    curr_month_idx = curr_date.month  # १ ते १२

    col_s1, col_s2 = st.columns(2)
    selected_year = col_s1.selectbox(
        "वर्ष निवडा:", [curr_year, curr_year - 1], index=0
    )
    upto_month = col_s2.selectbox(
        "कोणत्या महिन्यापर्यंत हिशोब पाहायचा आहे?",
        MONTHS_LIST,
        index=(curr_month_idx - 1),
    )

    upto_idx = MONTHS_LIST.index(upto_month) + 1
    # तपासण्याचे महिने
    months_to_check = [
        f"{m} {selected_year}" for m in MONTHS_LIST[:upto_idx]
    ]

    # सर्व पावत्या आणणे
    df_paid = pd.read_sql_query(
        "SELECT flat_no, month_year, amount FROM maintenance_received", conn
    )
    ...
    for _, flat in df_flats.iterrows():
      ...
      status = (
          "🔴 थकबाकी आहे" if pending_months_count > 0 else "🟢 सर्व भरले (Nil)"
      )
      months_str = (
          ", ".join(unpaid_months) if unpaid_months else "कोणताही महिना बाकी नाही"
      )
    ...
    c_m3.metric("सोसायटीची एकूण येणेबाकी (Dues)", f"₹ {total_society_dues:,.2f}")

    st.markdown("---")

    # फिल्टर पर्याय
    filter_choice = st.radio(
        "यादी कशी पाहायची आहे?:",
        ["फक्त थकबाकीदार फ्लॅट्स", "सर्व फ्लॅट्सची यादी"],
        horizontal=True,
    )
    if filter_choice == "फक्त थकबाकीदार फ्लॅट्स":
      display_df = df_dues_result[df_dues_result["एकूण बाकी महिने"] > 0]
    else:
      display_df = df_dues_result
    ...
    st.download_button(
        label="📥 हा थकबाकीदार अहवाल Excel मध्ये डाऊनलोड करा (.xlsx)",
        data=output_dues.getvalue(),
        file_name=(
            f"Rajgad_Flatwise_Dues_Report_{datetime.now().strftime('%d_%m_%Y')}.xlsx"
        ),
        mime=(
...
# ================= 6. अहवाल व डाऊनलोड =================
  ...
  rep_choice = st.radio(
      "कोणता अहवाल डाउनलोड करायचा आहे?",
      [
  ...
  if rep_choice == "१. संपूर्ण मेंटेनन्स जमा अहवाल (Maintenance Received)":
    ...
    st.download_button(
        "📥 मेंटेनन्स अहवाल Excel डाउनलोड करा",
        out.getvalue(),
    ...
    st.download_button(
      ...
      rep_choice == "३. एकत्रित ताळेबंद / नफा-तोटा (Profit & Loss Summary)"
  ):
    df_m = pd.read_sql_query(
        "SELECT SUM(amount) as total_maint FROM maintenance_received", conn
    )
    ...
    df_summary = pd.DataFrame({
        "तपशील": [
            "एकूण जमा मेंटेनन्स",
            "एकूण झालेला खर्च",
            "शिल्लक शिल्लक निधी (Net Balance)",
        ],
        "रक्कम (₹)": [tm, te, bal],
    })
    ...
    st.download_button(
        "📥 ताळेबंद समरी Excel डाउनलोड करा",
        out.getvalue(),
        "Balance_Summary.xlsx",
        "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
    )
